...
Completely rewrites MarkdownText.tsx with a correct implementation.

The previous patch broke it by replacing `text.` with `_text.` globally,
including inside inner helper functions that have their own `text` parameter.
This clean rewrite avoids that entirely.
"""
...
if not md:
    print("[ERROR] MarkdownText.tsx not found under", DASH)
    raise SystemExit(1)

print(f"Rewriting: {md}")

# Show current line count for reference
old = md.read_text(encoding="utf-8")
print(f"  Old file: {len(old.splitlines())} lines")
...
print("  [OK] MarkdownText.tsx cleanly rewritten")
print()
print("Next.js will hot-reload. Check /options Epsilon tab and /overview analysis panel.")
