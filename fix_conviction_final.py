# -*- coding: utf-8 -*-
"""
fix_conviction_final.py  -- Run from D:\MICC

PROBLEMS FOUND (from error log):
  1. Double navbar  -- layout.tsx has a ticker bar component that sits above
                       the page's own NavBar -> gap between them
  2. NameError LIMIT -- Python inside JS template literal sees SQL 'LIMIT ?' 
                        as Python name 'LIMIT' - fix: use .format() not f-string
  3. no such column peri -- period_label doesn't exist, real col = anchor_mm_dd
  4. is_today doesn't exist -- no such col, must compute from anchor_mm_dd
  5. WRONG layer cols -- symbol_conviction real schema:
       conviction_score, momentum_score, seasonality_score, quality_score,
       delivery_score, insider_score, news_score, fundamental_score,
       signal_count, top_reason
     NO beta_score, NO regime_score, NO n_layers, NO active_tags

Run: py D:\MICC\fix_conviction_final.py
"""
...
# ── Check layout.tsx for the ticker/double-bar ─────────────────────────────
print("\n[0] Checking layout.tsx...")
layout_path = DASH / "src" / "app" / "layout.tsx"
if layout_path.exists():
    lt = layout_path.read_text(encoding="utf-8")
    print(f"  layout.tsx imports: {[l.strip() for l in lt.splitlines() if 'import' in l and 'NavBar' in l or 'Ticker' in l or 'Bar' in l][:5]}")
    # Remove any NavBar from layout -- each page handles its own
    if "NavBar" in lt:
        import re
        lt2 = re.sub(r'import NavBar[^\n]*\n', '', lt)
        lt2 = re.sub(r'\s*<NavBar\s*/>', '', lt2)
        layout_path.write_text(lt2, encoding="utf-8")
        print("  [FIXED] Removed NavBar from layout.tsx")
    else:
        print("  [OK] No NavBar in layout.tsx")


# ── Write the API route with REAL column names ─────────────────────────────
print("\n[1] Writing /api/conviction/route.ts with real schema...")

# Key insight for the LIMIT bug:
# When Python f-string or template literal embeds SQL, `LIMIT` is fine as long
# as we don't use Python f-strings with {} around SQL params.
# Solution: build SQL as a plain JS string, pass params as JSON array to Python.
# The Python script uses conn.execute(sql, params) - no f-string collision.
...
# ── Write the page with correct layer names + single NavBar ───────────────
print("\n[2] Writing /conviction/page.tsx ...")
...
======================================================================
  ...
  Open: http://localhost:3000/conviction

  If double navbar still shows, run:
    py -c "
import re; p=open(r'D:\\MICC\\micc-dashboard\\src\\app\\layout.tsx').read()
print(p)
"
  and paste here.
""")
