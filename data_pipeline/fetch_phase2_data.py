...
======================================================
Adds sentiment, quality fundamentals, and historical options IV.
...
What this adds:
  ...
  3. News headlines stored + scored (feedparser + Ollama local LLM)
  4. Historical options IV surface (NSE bhavcopy backfill 2018→now)

Install:
  pip install requests beautifulsoup4 pytrends feedparser --break-system-packages

Run:
  py D:\MICC\data_pipeline\fetch_phase2_data.py              # all
  py D:\MICC\data_pipeline\fetch_phase2_data.py --screener   # fundamentals only
  py D:\MICC\data_pipeline\fetch_phase2_data.py --trends     # Google Trends only
  py D:\MICC\data_pipeline\fetch_phase2_data.py --news       # news storage only
  py D:\MICC\data_pipeline\fetch_phase2_data.py --iv         # options IV backfill
  py D:\MICC\data_pipeline\fetch_phase2_data.py --iv --from 2023-01-01  # partial
"""
...
# ═══════════════════════════════════════════════════════════════════════════════
# 3. NEWS HEADLINES — store, deduplicate, score
# ═══════════════════════════════════════════════════════════════════════════════

def create_news_table(c):
    c.execute("""
...
# NSE/India financial news RSS feeds
NEWS_FEEDS = [
...
# Symbol extraction — common NSE tickers that appear in headlines
NSE_KEYWORDS = {
...
def extract_symbols(text):
    """Extract NSE symbols mentioned in a headline."""
    text_lower = text.lower()
    found = []
    for symbol, keywords in NSE_KEYWORDS.items():
        if any(kw in text_lower for kw in keywords):
            found.append(symbol)
    return ",".join(found) if found else None

def score_sentiment_local(text):
    """
    Simple rule-based sentiment scoring.
    Returns -1.0 to +1.0
    For production: replace with Ollama call.
    """
    text_lower = text.lower()
    positive = ["surge", "rally", "gain", "jump", "rise", "bull", "buy",
                "profit", "growth", "strong", "record", "high", "up",
                "positive", "beat", "outperform", "upgrade"]
    negative = ["crash", "fall", "drop", "decline", "bear", "sell",
                "loss", "weak", "low", "down", "miss", "underperform",
                "downgrade", "concern", "risk", "warning", "trouble"]
...
def fetch_news_headlines():
    """Fetch and store financial news headlines from RSS feeds."""
    log("Fetching news headlines...")

    try:
        import feedparser
    except ImportError:
        log("pip install feedparser", "FAIL")
        return False
    ...
    for source, url in NEWS_FEEDS:
        try:
            ...
            for entry in feed.entries[:50]:  # max 50 per feed
                ...
                sentiment = score_sentiment_local(title)
    ...
    c.close()
    log(f"News headlines: {inserted} new articles stored", "OK")
    return True
...
def main():
    ...
    run_all = not any([args.screener, args.trends, args.news, args.iv])
    ...
    if run_all or args.news:
        results["News headlines"]        = fetch_news_headlines()
