...
Changes from broken version:
  - Fixed IndentationError (line 80 if block was empty)
  - Added pipeline_state.json — resume on partial failure
  - Added fallback Telegram on crash (sends yesterday's brief if engine fails)
  - Added --with-engine flag properly

Usage:
  cd D:/MICC/data_pipeline
  py run_pipeline.py                -> full update + snapshot + engine + Telegram
  py run_pipeline.py --with-engine  -> same (explicit)
...
def load_state():
    """Load today's pipeline state — skip already-completed phases."""
...
def save_state(state):
    try:
        ...
        log(f"Could not save pipeline state: {e}", "WARN")


def run_phase(name, script_path, desc, state, args=None, timeout=1800, cwd=None):
    """Run a phase, skip if already completed today, record result in state."""
...
def send_fallback_telegram(msg):
    """Send a plain Telegram message — used as crash fallback."""
    try:
        sys.path.insert(0, str(MICC_DIR))
        from micc_data import send_telegram
        send_telegram(msg)
    except Exception as e:
        log(f"Fallback Telegram failed: {e}", "WARN")


def main():
    weekly    = "--weekly"    in sys.argv
    no_engine = "--no-engine" in sys.argv
    # --with-engine is the default; both flags do same thing
    ...
    if not no_engine:
        ...
        if eng.exists():
            ...
            r["engine"] = ok
            if not ok:
                # Fallback: send yesterday's brief summary via Telegram
                log("Engine failed — sending fallback Telegram alert", "WARN")
                today_str = datetime.now().strftime("%d %b %Y")
                send_fallback_telegram(
                    f"⚠️ *MICC Pipeline Alert — {today_str}*\n"
                    f"Engine failed to run. Data phases: "
                    f"{sum(1 for v in r.values() if v)}/{len(r)} completed.\n"
                    f"Failed: {', '.join(state['failed'])}\n"
                    f"Run manually: `py D:\\MICC\\micc_engine.py 7 --send`"
                )
        else:
    ...
    if state["failed"]:
        log(f"To retry failed phases: py run_pipeline.py (will skip completed phases)", "WARN")
    print("=" * 65)
    print()
