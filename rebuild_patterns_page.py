# -*- coding: utf-8 -*-
"""
rebuild_patterns_page.py  --  Run from D:\\MICC
Complete rewrite of patterns/page.tsx with ALL features:
  - Existing: PatCard, YearChart (bar), RetDistChart, MonthHeatmap, WinDist, SummaryCards
  - NEW: InteractiveYearChart (overlay, all years colored lines, year filter)
  - NEW: Hover tooltip on ALL charts
  - NEW: Click-to-lock A/B comparison on ALL charts
  - NEW: 6 visualization tabs (Pattern Cards, Discovery Heatmap, 3D Surface, Bubble, Strip, Monthly)
  - NEW: Expandable chart panel buttons on Pattern Cards tab
  - NEW: Year x Day heatmap
  - NEW: HeatData API loaded in parallel
Run: py D:\\MICC\\rebuild_patterns_page.py
"""
...
print("\nRebuilding /patterns/page.tsx with ALL features...")
      ...
      {expanded&&(
        <div style={{padding:"0 16px 16px"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:14}}>
            <div>
              <div style={{fontSize:10,fontWeight:700,color:G,letterSpacing:1,marginBottom:6}}>SUCCESS ({succ.length})</div>
            ...
            <div>
              <div style={{fontSize:10,fontWeight:700,color:R,letterSpacing:1,marginBottom:6}}>FAILURE ({fail.length})</div>
              <div style={{display:"flex",flexWrap:"wrap",gap:4}}>
      ...
      <div style={{background:`${BOR}15`,borderRadius:10,padding:"12px 16px",marginBottom:14}}>
        <div style={{display:"flex",gap:14,flexWrap:"wrap",alignItems:"flex-start"}}>
          {[{l:"MIN ACCURACY",opts:[[0.70,"70%+"],[0.75,"75%+"],[0.80,"80%+"],[0.85,"85%+"],[0.90,"90%+"]],val:minAcc,set:setMinAcc},
            ...
            <div key={l}><div style={{fontSize:9,color:W9,fontWeight:700,letterSpacing:1,marginBottom:5}}>{l}</div>
          ...
          <div><div style={{fontSize:9,color:W9,fontWeight:700,letterSpacing:1,marginBottom:5}}>DIRECTION</div>
          ...
          <div><div style={{fontSize:9,color:W9,fontWeight:700,letterSpacing:1,marginBottom:5}}>WINDOW</div>
          ...
          <div><div style={{fontSize:9,color:W9,fontWeight:700,letterSpacing:1,marginBottom:5}}>SORT BY</div>
          ...
          <div><div style={{fontSize:9,color:W9,fontWeight:700,letterSpacing:1,marginBottom:5}}>YEAR GROUPS</div>
          ...
          {activeTab==="patterns"&&(
            <div>
              ...
              {showPats.length===0?(
                ...
                <div>
                  {activeDir==="both"&&upPats.length>0&&<div style={{marginBottom:20}}><div style={{fontSize:13,fontWeight:700,color:G,letterSpacing:1,marginBottom:8}}>UP PATTERNS ({upPats.length})</div>{upPats.map((p,i)=><PatCard key={`up-${i}`} p={p} compareYears={compareYears} symbol={data.symbol} assetType={data.asset_type}/>)}</div>}
                  {activeDir==="both"&&dnPats.length>0&&<div><div style={{fontSize:13,fontWeight:700,color:R,letterSpacing:1,marginBottom:8}}>DOWN PATTERNS ({dnPats.length})</div>{dnPats.map((p,i)=><PatCard key={`dn-${i}`} p={p} compareYears={compareYears} symbol={data.symbol} assetType={data.asset_type}/>)}</div>}
...
print("\nSYNTAX CHECK...")
import ast, sys
src = (APP / "patterns" / "page.tsx").read_text(encoding="utf-8")
# Basic checks
issues = []
if src.count("useState") < 5: issues.append("Too few useState")
if "export default function PatternsPage" not in src: issues.append("Missing main export")
if "OverlayChart" not in src: issues.append("Missing OverlayChart")
if "YearChart" not in src: issues.append("Missing YearChart")
if "openPanels" not in src: issues.append("Missing openPanels")
if issues:
    print(f"ISSUES: {issues}")
else:
    print("All checks passed!")

print(f"\nFile size: {len(src):,} chars, {src.count(chr(10))} lines")
print("""
=============================================================
COMPLETE. Restart dashboard:
  cd D:\\MICC\\micc-dashboard && npm run dev
  
Then open localhost:3000/patterns and search HDFCBANK.

ALL FEATURES NOW IN ONE FILE:
  PatCard — click to expand with:
    - Stats grid (12 stats)
    - Return distribution dot plot
    - Year-by-year BAR CHART (hover tooltip + click A/B lock)
    - Success/failure year lists
    - OVERLAY CHART (all years as colored lines)
      + Year filter (Last 3/5/7/10/15yr)
      + Toggle individual years on/off
      + Hover tooltip (year/day/return)
      + Click-to-lock A & B points with Δ comparison
      + Average (white) + Median (yellow dashed) lines
    - Year x Day heatmap

  6 TABS:
    📋 Pattern Cards (with expandable Heatmap/Bubble/Strip/Monthly panels)
    🗺 Discovery Heatmap (Month x Window grid)
    🏔 3D Surface (same grid, cleaner rendering)
    🫧 Bubble Chart (Accuracy vs Return)
    📅 Calendar Strip (year timeline)
    📊 Monthly Heatmap

  EXPANDABLE PANELS (on Pattern Cards tab):
    Buttons: 🗺 Heatmap | 🫧 Bubble | 📅 Strip | 📊 Monthly
    Click to open/close above the pattern list
=============================================================
""")
